...
- **Changed plans mid-month?** GitHub resets its AI-usage counter when you switch plans, but the billing API keeps returning the whole calendar-month total — so the card would over-count (e.g. show 9,041 while the dashboard shows 3,915). Use **"Adjust usage"** in the Copilot settings: enter the current value from GitHub's dashboard once, and the app subtracts the pre-change amount (stored as a number + month tag only) so the card matches the dashboard. It clears automatically at the start of the next month.
