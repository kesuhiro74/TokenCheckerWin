...
- UI 言語の切り替え(System 連動 / English / 日本語・再起動で反映)
- Windowsログイン時の自動起動設定
...
`tests/` 配下に xUnit テストプロジェクトがあり、アプリの不変条件(80%/95% 閾値・プライバシーマスキング・Copilot allowance・予測ロジック・設定マイグレーション・プロバイダ失敗分離など)を固定しています。コミット・リリース前にソリューション全体でグリーンであることを確認してください。

```powershell
dotnet test
```

GitHub Actions(`.github/workflows/ci.yml`)が push / PR ごとに windows-latest 上で `dotnet test` を実行します。
...
- 外部画像を持たず、起動時にコードでアイコンを生成します。有効なウィンドウごとにアイコンを表示します(Claude / Codex は縦%バー、GitHub Copilot も縦%バー)。
- Claude / Codex の縦%バーは、有効なサービスの最大 `usedPercent` を縦に伸びるバーで示し、トレイを一目見るだけで最も逼迫しているウィンドウの目安が分かります。Claude / Codex の両方が有効なら 2 本、片方だけなら 1 本を表示します。
- 全体最悪値に応じて配色が変わります。
  - 通常 (`< 80%`): サービスのブランド色
  - 警告 (`>= 80%`): アンバー
  - 危険 (`>= 95%`): 赤
  ...
  - 取得前 / データなし: ライトグレー
- マウスオーバーのツールチップは UI 言語に追従し、サービスごとのステータスバッジ、または `5h X% / Weekly Y%` を表示します。
...
- 設定ダイアログの `認証状態を再確認` は単に使用率取得を再実行します。ログインが成功していればステータスが `正常取得` に切り替わり、トレイアイコンも更新されます。
...
- ステータスバッジは UI 言語に追従します(日本語では `正常取得` / `未インストール` / `未ログイン` / `認証エラー` / `取得を一時制限中` / `取得失敗` / `状態不明`)。`ProviderStatus` enum の英名はそのまま画面に出しません。`取得を一時制限中` は usage endpoint 自体が HTTP `429` を返したときだけ表示されます(LLM 自体のクォータ超過ではないことに注意)。
...
- `settings.json` には、更新間隔・自動起動設定・テーマ(`ThemeMode`: システム連動 / ライト / ダーク)・UI 言語(`Language`: System 連動 / English / 日本語)・表示モード(`DisplayMode`、後方互換のための `CompactMode`)・各ウィンドウの表示 ON/OFF・表示方法(`常時表示` / `ホバー表示`)・Claude / Codex の表示対象サービス・GitHub Copilot のプラン / Custom 上限・配色・各ウィンドウの位置といった**アプリ設定のみ**を保存します。トークン・認証情報・login・URL・パス・メールアドレスは一切保存しません。
...
- **UI 言語(System 連動 / English / 日本語)** を「共通設定」で選べます。`System` は Windows の表示言語(UI カルチャ)が日本語なら日本語、それ以外は英語になります。テーマと同じく**反映は起動時のみ**で、変更後は**アプリを再起動**すると全 UI(ウィンドウ・設定ダイアログ・トレイメニュー・ツールチップ)に反映されます(設定画面にも「(再起動で反映)」と表示)。
...
- **トレイアイコン**は有効なウィンドウごとに表示します(Claude / Codex は縦%バー、GitHub Copilot も縦%バー)。Copilot の縦%バーはアイコン領域を縦長に使い、当月消費の割合(プラン上限比)を示します(角はやや角張った角丸矩形・80%/95% の色変化は維持)。両ウィンドウとも OFF のときは、設定・終了に到達できるよう操作用アイコンを 1 つだけ表示します。
- **配色**: GitHub Copilot の**カードのバー・トレイ縦%バーの通常色(80% 未満)**を設定で選べます(`ブルー（既定）` / `グリーン` / `スカイ` / `パープル` / `スレート`)。**数値(`n%`)は黒系・`使用済み` は薄いグレーで固定**(配色設定の影響を受けません)。**80% 以上で橙・95% 以上で赤へのエスカレーションはバー側で維持**されます(Claude / Codex と同じ閾値)。<br>※配色設定はバーにのみ反映されます。以前のビルドで `スレート` 等を選んでいた場合、本バージョンからはバーがその色になります。
