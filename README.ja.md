...
- **「クレジット」見出し行**: パーセントのすぐ上に、左に `クレジット`、右に月次リセット目安(例: `リセット目安 7/1`)を小さく表示します。
- **本日の増分(2 番目の主役表示)**: 本日 9:00 以降の増分を、スパークアイコン＋小さな `本日` ラベル(上の「使用済み」と同じサイズ・色)＋大きめの数値(上の「67%」と同じサイズ・色)で表示します(例: `✦ 本日 +96（+1.4%）`、基準値が無ければ `本日: 未計測`)。**スパークアイコンだけ**が**当日ペース**に応じて色を変え、4% 未満は緑、4〜5% は橙、5% 以上は赤になります(1 日あたりの消費ペースの指標で、月次の 80% / 95% 閾値とは別概念です)。その下に、このペースで 100% に到達する予測日(例: `このペースだと 6/21 頃に 100%`、不能時は `予測にはデータ不足`)を小さく表示します。
...
- **トレイアイコン**は有効なウィンドウごとに表示します(Claude / Codex は縦%バー、GitHub Copilot も縦%バー)。Copilot の縦%バーはアイコン領域を縦長に使い、当月消費の割合(プラン上限比)を示します(角はやや角張った角丸矩形・80%/95% の色変化は維持)。さらに、本日のバーン(9:00 以降の当日消費)が高い日は、バーに大きな警告スパークが重なります(4〜5% で橙・5% 以上で赤、4% 未満は表示なし。カードのスパークと同じ当日ペースの閾値で、月次の 80/95% とは別概念)。両ウィンドウとも OFF のときは、設定・終了に到達できるよう操作用アイコンを 1 つだけ表示します。
