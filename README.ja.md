...
<img width="549" height="46" alt="image" src="https://github.com/user-attachments/assets/7d5bb640-c8e7-4d2b-bccb-29c50451c033" /><br>
...
- **本日の推定利用額**(`¥N (daily)`): ローカルの Claude / Codex セッションログから当日のトークンを集計し、内蔵のモデル別単価表で金額化して円換算します(USD→JPY レートは 1 日 1 回、公開為替 API から取得)。通常カードとミニマム行に表示し、算出できないときは非表示にします。
...
- **ミニマム (Minimum)** — サービスごとに 1 行の Nerd Font ステータスライン: `<アイコン> Claude | 5h 38% 10:50 | 7d 39% 6/17 18:00 | ¥46 (daily)`(サービス名・5時間/週次の `%` とそれぞれのリセット時刻・本日の利用額)。文字は Cascadia Mono、アイコンは Symbols Nerd Font(未インストールならアイコンは省略)で描画します。`|` は 2 行で縦に揃え、ウィンドウは行幅に自動フィットします。データ不明のセグメント(リセット時刻なし・金額が算出不能など)はそのぶん省略します。
...
- **テーマ**(ライト / ダーク / システム連動)と **UI 言語**(System / English / 日本語)は**起動時のみ反映**されます。変更後はアプリを再起動してください(ダイアログにも「(再起動で反映)」と表示)。`System` は Windows の色モード / 表示言語に追従します。
...
- **本日の増分**を 2 番目の主役として表示します。本日 9:00 以降の増分(例: `✦ 本日 +96（+1.4%）`、基準値が無ければ `本日: 未計測`)。スパークアイコンは**当日ペース**で色を変えます: 4% 未満は緑、4〜5% は橙、5% 以上は赤(月次の 80/95% 閾値とは別概念)。高い日は同じ警告スパークがトレイアイコンにも重なります。その下に 100% 到達予測日(例: `このペースだと 6/21 頃に 100%`、不能時は `予測にはデータ不足`)。
