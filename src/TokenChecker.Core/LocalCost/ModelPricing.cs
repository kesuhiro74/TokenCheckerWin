...
// against the official pricing pages as of 2026-09; verify against official
...
public static class ModelPricing
{
    ...
    private static readonly ModelPrice[] Prices =
    {
        ...
        // The *-5-1 entries must outrank their "-5" prefixes: 5.1 cache reads
        // are 0.025x input instead of the standard 0.1x.
        new("claude-fable-5-1", 10m, 50m, 12.5m, 0.25m),
        new("claude-fable-5", 10m, 50m, 12.5m, 1m),
        new("claude-mythos-5-1", 10m, 50m, 12.5m, 0.25m),
        new("claude-mythos", 10m, 50m, 12.5m, 1m),
        // Opus 5.5 cache reads are 0.05x input.
        new("claude-opus-5-5", 4m, 20m, 5m, 0.2m),
        new("claude-opus-5", 5m, 25m, 6.25m, 0.5m),
        ...
        new("claude-sonnet-5", 2m, 10m, 2.5m, 0.2m),
        new("claude-sonnet-4", 3m, 15m, 3.75m, 0.3m),
        new("claude-haiku-4", 1m, 5m, 1.25m, 0.1m),
        new("claude-3-5-haiku", 0.8m, 4m, 1m, 0.08m),

        // OpenAI. GPT-5.6 rows use the standard short-context rates and list a
        // cache-write price; earlier models don't charge for cache writes (0).
        // Codex logs report no cache writes, so that column is informational.
        new("gpt-5.6-sol", 4m, 20m, 5m, 0.4m),
        new("gpt-5.6-terra", 2m, 12m, 2.5m, 0.2m),
        new("gpt-5.6-luna", 0.2m, 1.2m, 0.25m, 0.02m),
